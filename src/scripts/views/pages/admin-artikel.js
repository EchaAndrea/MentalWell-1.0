  ...
  try {
    ...
    if (res.ok && result.data) {
      allArticles = result.data.map(a => ({
        ...
        judul: a.title,
        kategori: a.category,
        tanggal: a.date ? new Date(a.date).toLocaleDateString('id-ID', { day: '2-digit', month: 'long', year: 'numeric' }) : '',
...
function handleSearch() {
  ...
  filteredData = allArticles.filter(p =>
  ...
  if (konfirmasi.isConfirmed) {
    try {
      ...
      if (res.ok && result.status === 'success') {
        Swal.fire({ icon: 'success', title: 'Berhasil', text: 'Artikel dihapus.' });
        await fetchArticles();
        renderKategoriOptions();
        renderTable();
      } else {
        Swal.fire({ icon: 'error', title: 'Gagal', text: result.message || 'Gagal menghapus.' });
      }
    } catch (err) {
...
async function hapusYangDipilih() {
  const checked = Array.from(document.querySelectorAll('.row-checkbox:checked')).map(cb => cb.dataset.id);
  if (checked.length === 0) {
    Swal.fire({ icon: 'info', title: 'Tidak ada yang dipilih', text: 'Pilih artikel yang ingin dihapus.' });
    return;
  }
  const konfirmasi = await Swal.fire({
    icon: 'warning',
...
window.resetTable = () => { document.getElementById("searchInput").value = ""; handleSearch(); };
