    ...
    async function () {
      ...
      try {
        const res = await fetch(
          `https://mentalwell10-api-production.up.railway.app/counselings/${psikologId}`,
          {
            method: "POST",
        ...
        if (data.status === "success") {
          ...
          Swal.close();
          window.location.href = `/jadwalkonseling-selesai.html?id=${psikologId}&mode=schedule`;
        } else {
          Swal.close();
          Swal.fire({
            icon: "error",
            title: "Pembayaran Gagal",
            text: data.message || "Gagal mengirim pembayaran schedule",
          });
        }
      } catch (error) {
        ...
        Swal.fire({
          icon: "error",
          title: "Pembayaran Gagal",
          text: "Gagal memproses pembayaran schedule. Silakan coba lagi.",
        });
      }
    };
});
